...
const getSquarePosition = (week: number) => {
...
const Square = ({ week, isFull }: { week: number; isFull: boolean }) => {
  const { x, y } = getSquarePosition(week);
  const fill = isFull ? "#cbd5e1" : "#f8fafc";
  const stroke = "#e2e2e2";
  const strokeWidth = 1;
  const squareWithGap = SQUARE_SIZE + 2; // add 2 for gap
  const squareX = x + 1; // add 1 for gap
  const squareY = y + 1; // add 1 for gap
  return (
    <>
      <rect
        x={squareX}
...
export const Grid = ({
  ...
  console.log(weeksfromBorn, weeksLeft)
  return (
    <svg viewBox={`0 0 ${52 * SQUARE_SIZE} ${Math.ceil(90) * SQUARE_SIZE}`}>
      <Square week={0} isFull={true} />
      {Array.from({ length: weeksfromBorn - 1 }, (_, i) => (
        <Square key={i + 1} week={i + 1} isFull={true} />
      ))}
      {Array.from({ length: weeksLeft }, (_, i) => (
        <Square key={i + weeksfromBorn} week={i + weeksfromBorn} isFull={false} />
      ))}
    </svg>
  );
};
...
export const Grid = ({
  ...
  const emptySquare = (
    <svg
      ...
      viewBox="0 0 130 120.48076923076923"
      width="12"
      height="12"
    >
      <defs></defs>
      ...
      <g
        ...
        transform="translate(10 10) rotate(0 55 50.24038461538461)"
      >
        <path
  ...
  const fullimages = Array.from({ length: weeksfromBorn }).map((_, i) => (
    <SquareImage key={i} svg={fullSquare} />
  ));

  const emptyImages = Array.from({ length: weeksLeft }).map((_, i) => (
    <SquareImage key={i} svg={emptySquare} />
  ));
...
const SquareImage = ({ svg }: { svg: JSX.Element }) => {
  return svg;
};
 */